...
Scales of sex-chromosome expression for expression-based sex determination: `r params$ContRatio`
...
We have drawn a line `r if(params$ContRatio == "fixed"){"corresponding to the x-y diagonal"} else {"from the origin to the respective maximum values of the x and y axes"}`. If you observe clear separation between males and females, and that the line does not capture this separation well, you may adjust the Slope of the line using the
...
Samples in the shaded area are automatically removed. The area is defined by an angle of `r params$Cont` degrees centered over the x-y diagonal. This is reasonable default, however you can adjust this with `--ContaminationArea` flag, if needed. The ratio between x and y axes have been set to be `r params$ContRatio`. This can be changed with `--XyExpressionRatio`.
